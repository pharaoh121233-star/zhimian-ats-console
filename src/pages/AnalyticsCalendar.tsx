...
export function AnalyticsDetail() {
  ...
  return <div><PageHeader title={`${metric}明细`} description="由数据看板下钻，已自动带入来源筛选条件" extra={<Button icon={<LeftOutlined />} onClick={()=>navigate(-1)}>返回来源看板</Button>} /><Alert type="info" showIcon message={`筛选摘要：来源 ${params.get('source')||'招聘总览'} · 日期 2026-08-01 至 2026-09-03 · 全部项目`} /><Row gutter={12} className="stats-row"><Col span={6}><StatCard label="明细记录" value="386" /></Col><Col span={6}><StatCard label="涉及项目" value="12" /></Col><Col span={6}><StatCard label="涉及岗位" value="28" /></Col><Col span={6}><StatCard label="数据更新时间" value="16:50" /></Col></Row><Card><Table dataSource={interviews} columns={[{title:'面试编号',dataIndex:'code'},{title:'候选人',dataIndex:'candidate'},{title:'项目',dataIndex:'project'},{title:'岗位',dataIndex:'job'},{title:'轮次',dataIndex:'round'},{title:'状态',dataIndex:'status',render:(v)=><StatusTag status={v}/>},{title:'更新时间',dataIndex:'updated'}]} /></Card></div>;
}

const events:Record<string,{title:string;status:string;time:string;candidate:string}[]> = {
  '2026-09-03':[{title:'云产品技术支持工程师 · 二轮复试',status:'已确认',time:'10:00',candidate:'程砚秋'},{title:'客户服务组长 · 客户面',status:'冲突',time:'14:30',candidate:'林望舒'},{title:'解决方案支持顾问 · 终试',status:'待确认',time:'16:00',candidate:'周景澄'}],
  '2026-09-08':[{title:'售后服务专员 · 人工复试',status:'已确认',time:'15:00',candidate:'江予安'}],
  '2026-09-16':[{title:'交付项目协调专员 · 客户面',status:'待确认',time:'11:00',candidate:'孟书瑶'}],
};

export function CalendarPage() {
  const [selected,setSelected]=useState<Dayjs>(dayjs('2026-09-03')); const [drawer,setDrawer]=useState(false); const [create,setCreate]=useState(false);
  return <div><PageHeader title="面试日历" description="仅展示二轮及后续人工面试、客户面和排班环节" extra={<Button type="primary" onClick={()=>setCreate(true)}>新建人工面试</Button>} />
    ...
    <Drawer open={drawer} onClose={()=>setDrawer(false)} title="人工面试安排" width={520}><Alert type="error" showIcon message="检测到面试官时间冲突" description="陈砚在 14:00 - 15:00 已有另一场客户面。" /><Form layout="vertical" style={{marginTop:16}}><Form.Item label="候选人"><Input value="林望舒" readOnly/></Form.Item><Form.Item label="时间"><DatePicker showTime value={dayjs('2026-09-03 14:30')}/></Form.Item><Form.Item label="面试官"><Select mode="multiple" defaultValue={['陈砚','梁序']} options={[{value:'陈砚',label:'陈砚（冲突）'},{value:'梁序',label:'梁序（空闲）'}]}/></Form.Item><Form.Item label="同步日历"><Checkbox defaultChecked>同步飞书日历并重新通知参会人</Checkbox></Form.Item><Button type="primary" block onClick={()=>message.success('改期已保存，通知已重新发送')}>确认改期</Button></Form></Drawer>
