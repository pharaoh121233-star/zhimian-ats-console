  ...
  return <div>
    ...
    <Card className="invite-card">
      {current===0 && <div><div className="section-title"><h2>选择候选人与岗位</h2><span>已选择 {selected.length} 人</span></div>
        ...
        <Alert type="warning" showIcon message="查重提醒" description="韩龙的邮箱与已有候选人记录匹配，请人工确认后再发起邀约。" action={<Button size="small">查看重复记录</Button>} />
      ...
      {current===2 && <Row gutter={20}><Col span={15}><Card size="small" title="接收人清单"><List dataSource={candidates.slice(0,2)} renderItem={(x,i)=><List.Item><Space><CheckCircleFilled className="success-icon" /><b>{x}</b><span>{interviews[i]?.email}</span><Tag>{interviews[i]?.job}</Tag></Space></List.Item>} /></Card><Alert style={{marginTop:16}} type="warning" showIcon message="发现 1 项重复邀约风险" description="韩龙已有同岗位面试记录，继续发起将保留两次发送记录。" /></Col><Col span={9}><Card size="small" title="本次邀约摘要"><Descriptions column={1} size="small"><Descriptions.Item label="候选人数">2 人</Descriptions.Item><Descriptions.Item label="短信">0 条</Descriptions.Item><Descriptions.Item label="邮件">2 封</Descriptions.Item><Descriptions.Item label="链接有效期">48 小时</Descriptions.Item><Descriptions.Item label="预计费用">¥ 0.00</Descriptions.Item></Descriptions></Card></Col></Row>}
...
export function ProcessPage() {
  ...
  return <><BusinessList title="进程管理" description="实时跟踪面试、链接、评分和异常状态" stats={[
    {label:'全部记录',value:interviews.length},{label:'邮件已发送',value:interviews.filter(item=>item.linkStatus==='邮件已发送').length},{label:'已完成',value:interviews.filter(item=>item.status==='已完成').length,tone:'green'},{label:'建议通过',value:interviews.filter(item=>item.score>=70).length,tone:'green'},{label:'待复核',value:interviews.filter(item=>item.result==='待复核').length,tone:'orange'},{label:'低于 60 分',value:interviews.filter(item=>item.score<60).length,tone:'red'},
...
export function ReviewPage() {
  const [detail,setDetail]=useState<typeof interviews[number]>();
  const [filters,setFilters]=useState<InterviewFilterValue>({});
  const items=useMemo(()=>applyInterviewFilters(interviews.filter(x=>x.result==='待复核'),filters),[filters]);
  return <div><PageHeader title="待审核" description="审核 AI 面试结果并推进候选人流程" extra={<Radio.Group defaultValue="card" optionType="button" options={[{value:'card',label:'卡片'},{value:'table',label:'表格'}]} />} />
    ...
    <Row gutter={[14,14]}>{items.map(x=><Col span={8} key={x.key}><Card className="candidate-card" hoverable onClick={()=>setDetail(x)}><div className="candidate-top"><div className="candidate-avatar">{x.candidate.slice(-1)}</div><div><h3>{x.candidate}</h3><p>{x.job}</p></div><b className="score">{x.score}</b></div><Divider /><Space wrap><StatusTag status={x.result} /><Tag>{x.project}</Tag><Tag>AI 首轮</Tag></Space><div className="candidate-meta"><span>{x.email}</span><span>{x.interviewTime}</span></div></Card></Col>)}</Row>
    <Drawer open={!!detail} onClose={()=>setDetail(undefined)} width="88%" title={`候选人审核 · ${detail?.candidate||''}`}><Row gutter={24}><Col span={15}><Tabs items={['AI 摘要','面试转写','录音录像','题目与回答','简历'].map((x,i)=>({key:String(i),label:x,children:<Card><Alert type="info" showIcon message={`AI 综合得分：${detail?.score||0} 分`} description={`候选人应聘 ${detail?.job||''}，当前结果为${detail?.result||'待复核'}。建议结合完整回答记录进行人工判断。`} /><Divider /><h3>候选人信息</h3><p>邮箱：{detail?.email}</p><p>完成时间：{detail?.interviewTime}</p><h3>审核关注项</h3><p>重点复核岗位匹配度、回答完整性和实际工作经验。</p></Card>}))} /></Col><Col span={9}><Card title="分项评分"><List dataSource={[['岗位匹配',detail?.score||0],['表达完整',Math.max(0,(detail?.score||0)-4)],['业务理解',Math.max(0,(detail?.score||0)-8)],['稳定性',Math.min(100,(detail?.score||0)+3)]]} renderItem={x=><List.Item><span>{x[0]}</span><Progress percent={Number(x[1])} style={{width:160}} /></List.Item>} /><Divider /><Form layout="vertical"><Form.Item label="审核意见"><Input.TextArea rows={4} placeholder="填写审核意见" /></Form.Item><Space direction="vertical" style={{width:'100%'}}><Button block type="primary" onClick={()=>message.success('已通过并进入下一轮')}>通过并进入下一轮</Button><Button block>直接通过</Button><Button block danger onClick={()=>Modal.confirm({title:'驳回并释放到公海？',content:<Input.TextArea placeholder="驳回原因（必填）" />,okText:'确认驳回'})}>驳回并释放公海</Button></Space></Form></Card></Col></Row></Drawer>
  </div>;
}
...
export function PassedPage() {
  ...
  const passedRecords=interviews.filter(x=>x.score>=70);
  return <><BusinessList title="面试通过" description="评估是否结束流程，或创建飞书视频会议发起二轮人工面试" stats={[{label:'待评估',value:passedRecords.length},{label:'80 分及以上',value:passedRecords.filter(item=>item.score>=80).length},{label:'70-79 分',value:passedRecords.filter(item=>item.score>=70&&item.score<80).length},{label:'面试完成',value:interviews.length,tone:'green'}]} data={passedRecords.map(x=>({...x,status:'待决定'})) as unknown as RecordType[]} columns={[textCol('候选人','candidate'),textCol('项目 / 岗位','job',220),textCol('当前轮次','round'),{title:'AI 总分',dataIndex:'score',width:100},statusCol(),textCol('负责人','owner'),textCol('更新时间','updated')]} primaryAction="批量评估" filterNames={['候选人 / 面试编号','项目','岗位','负责人','当前轮次','通过状态']} rowActions={record=><Button type="link" size="small" icon={<VideoCameraOutlined />} onClick={()=>{setTarget(record);setDecision(Number(record.score)>=80?'complete':'second');}}>评估去向</Button>} />
